...
path = 'D:\\Documents\\VS Code\\Python\\PyGame\\PyGame-Clean Code\\PygameParticiles\\'
...
class ParticleStar:
    def __init__(self):
        self.particles = []
        self.surface = pygame.image.load(path + 'Star.png').convert_alpha()
    ...
    def add_particles(self):
        ...
        particle_rect = pygame.Rect(
            int(pos_x), int(pos_y), self.width, self.height)
        self.particles.append(
            [particle_rect, direction_x, direction_y, lifetime])

    def delete_particles(self):
        particle_copy = [
            particle for particle in self.particles if particle[3] > 0]
        self.particles = particle_copy
...
nyan_surface = pygame.image.load(path + 'nyan_cat.png').convert_alpha()
